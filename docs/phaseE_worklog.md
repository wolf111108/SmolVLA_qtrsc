...
### 6. E3 组件级噪声容忍度实验(5×5 矩阵,已启动,进行中)

- **目的**:测量 vlm 与 expert 两个组件**分别**注噪时的闭环 SR 衰减,
  得到各组件的噪声容忍度(tolerance)曲线
- **设计**:vlm α_v ∈ {0.01, 0.05, 0.10, 0.20, 0.50} × expert α_e ∈ 同档位,
  共 25 组;每组对全部 288 site 同时注噪(vlm 144 个用 α_v,expert 144 个用 α_e),
  方法 `gaussian_rms_output`,seed=0,libero_object 10 episodes
- **实现改动**:
  - `src/vla_tcs2/model_wrapper.py` — `apply_sensitivity_target()` 新增
    `test.targets`(复数)多组选择器支持:每组独立 `target` + 可覆盖
    `alpha`/`seed`/`method` 等;组内键覆盖全局 `test.*` 默认值;
    **保持单 target 旧 config 完全向后兼容**
  - `scripts/gen_phaseE3_configs.py` — 生成 25 个 config
    (`phaseE3_v{αv×100}_e{αe×100}`,如 `phaseE3_v05_e50` = vlm α=0.05 + expert α=0.50)
  - `scripts/run_phaseE3_all.sh` — 顺跑 driver(断点续跑,末尾输出 5×5 SR 网格)
- **验证**(干跑脚本,已删除):`component=vlm/expert` 各匹配 144 模块;
  多组 α 分组正确(vlm 全部 0.1、expert 全部 0.5);单 target 旧路径
  匹配 1/288 正常;25 个 config 解析与 α 编码全部断言通过;
  实跑日志确认 `-> 288 modules in test_forward`
- 启动时间:2026-09-08 09:58(driver PID 1899082),预计 ~4 小时
- 日志:`outputs/phaseE3_driver.log` + 每个 `outputs/phaseE3_<name>.log`
- 基线参照:E2 baseline_raw SR=94%(同设置,10 episodes)

**5×5 SR 网格(行=α_vlm,列=α_expert,跑完后填写):**

| α_v \ α_e | 0.01 | 0.05 | 0.10 | 0.20 | 0.50 |
|---|---|---|---|---|---|
| 0.01 | — | — | — | — | — |
| 0.05 | — | — | — | — | — |
| 0.10 | — | — | — | — | — |
