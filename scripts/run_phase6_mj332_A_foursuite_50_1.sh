...
# 输出: ~/VLA_tcs2/outputs/table2_repro_audit/06_simulator/mj332_A_na1_ep50_<suite>/eval_info.json
#
# 注意:
...
LOGDIR="$BASE_OUT/mj332_A_na1_ep50_logs"
mkdir -p "$LOGDIR"

SUITES=("libero_spatial" "libero_object" "libero_goal" "libero_10")

for SUITE in "${SUITES[@]}"; do
    OUT="$BASE_OUT/mj332_A_na1_ep50_${SUITE}"
...
for suite in ["libero_spatial", "libero_object", "libero_goal", "libero_10"]:
    f = base / f"mj332_A_na1_ep50_{suite}/eval_info.json"
    if not f.exists():
        print(f"{suite}: 未完成")
        continue
