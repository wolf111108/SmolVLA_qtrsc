#!/usr/bin/env bash
# 消融补充 — A 类 lerobot/smolvla_libero 四 suite，n_action_steps=50, n_episodes=50
# 目的：补齐消融矩阵的 na=50（=chunk_size 边界）数据点，观察 n_action_steps 在 10 → 30 → 50 是否继续提升或饱和
#
# 用法: 在 smolvla_eval 环境执行: bash run_phase6_mj332_A_foursuite_na50_ep50.sh
# 输出: ~/VLA_tcs2/outputs/table2_repro_audit/06_simulator/mj332_A_na50_ep50_<suite>/eval_info.json
#
# 注意:
...
# - na=50 是 chunk_size 上限，执行完整 action chunk（一次推理覆盖整个 horizon）
...
for suite in ["libero_spatial", "libero_object", "libero_goal", "libero_10"]:
    f = base / f"mj332_A_na50_ep50_{suite}/eval_info.json"
    if not f.exists():
        print(f"{suite}: 未完成")
        continue
