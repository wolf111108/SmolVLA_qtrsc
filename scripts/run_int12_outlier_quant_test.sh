#!/usr/bin/env bash
# int12 + outlier 保护 量化测试：校准 + 量化推理 eval
#
# 用法：
#   bash scripts/run_int12_outlier_quant_test.sh              # 校准 + eval
#   bash scripts/run_int12_outlier_quant_test.sh --reuse      # 跳过校准，复用已有 scales
#
# 说明：activation/weight/output 统一 int12，启用 outlier_ratio（见 config）。

set -euo pipefail

CONFIG=configs/experiments/smolvla_int12_outlier_quant_test.yaml
OUT_DIR=outputs/experiments/smolvla_int12_outlier_quant_test
...
echo "==================== int12 + outlier quantization test ===================="
echo "config : $CONFIG"
echo "output : $OUT_DIR"
echo "==========================================================================="
...
sr = 100 * ok / n
print(f"int12+outlier   : {ok}/{n} = {sr:.1f}%")
