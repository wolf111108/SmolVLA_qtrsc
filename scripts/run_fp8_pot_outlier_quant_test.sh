#!/usr/bin/env bash
# PoT-FP8 + Outlier Protection：运行 calibration + LIBERO eval + 结果/scale 审计
#
# 默认配置：
#   configs/experiments/smolvla_fp8_pot_outlier_quant_test.yaml
#
# 用法：
#   # 1) 首次：重新校准 + evaluation + 收集结果
#   bash scripts/run_fp8_pot_outlier_quant_test.sh
#
#   # 2) 复用已有 PoT scales，只做 evaluation + 收集结果
#   bash scripts/run_fp8_pot_outlier_quant_test.sh --reuse
#
#   # 3) 只重新汇总已有 result.json / scale 文件，不运行模型
#   bash scripts/run_fp8_pot_outlier_quant_test.sh --summary-only
#
#   # 4) 指定配置 / FP baseline
#   bash scripts/run_fp8_pot_outlier_quant_test.sh \
#       --config configs/experiments/smolvla_fp8_pot_outlier_quant_test.yaml \
#       --baseline 93.8
#
# 输出（写入 YAML 的 output_dir）：
#   result.json                 main.py 原始 evaluation 结果
#   config.yaml                 main.py 保存的配置快照
#   run_YYYYmmdd_HHMMSS.log     完整运行日志
#   pot_scale_audit.csv         每个 scale 的 value / exponent / 是否 PoT
#   pot_scale_audit.json        scale 审计机器可读结果
#   per_task_sr.csv             每 task success rate
#   summary.json                本次实验摘要
#   summary.md                  人类可读摘要
#
# 说明：
#   - 第一版沿用当前 dynamic outlier protection，只把 scale 约束为 2^k。
#   - quantize_matmul=false 时这里只审计 Linear 的 a/w/o scale。
#   - --reuse 必须确保 scale_dir 中已有本方法生成的 PoT scale；
#     脚本会在运行前后检查，发现非 2^k scale 会报错。
#
set -euo pipefail

CONFIG="configs/experiments/smolvla_fp8_pot_outlier_quant_test_full.yaml"
FP_BASELINE="93.8"
REUSE=0
SUMMARY_ONLY=0
...
echo "================================================================================"
echo "                    PoT-FP8 + Outlier Protection Test"
echo "================================================================================"
echo "config          : $CONFIG"
...
# ---------------------------------------------------------------------------
# Preflight: the new methods must actually be registered, and YAML must use FP8.
# ---------------------------------------------------------------------------
python - "$CONFIG" <<'PY'
import sys, yaml
...
print(f"[preflight] scale method registered: {name}")
print(f"[preflight] quant method registered: {name}")
print("[preflight] FP8/passthrough precision fields: PASS")
PY

# ---------------------------------------------------------------------------
# Helper: audit scale pickle files and enforce exact power-of-two property.
# Can run before --reuse and always runs after the experiment.
# ---------------------------------------------------------------------------
audit_scales() {
...
summary = {
    ...
    "experiment": "PoT-FP8 + Outlier Protection",
...
md = []
md.append("# PoT-FP8 + Outlier Protection — Result Summary")
md.append("")
...
md.append("")
md.append(f"- PoT-FP8 + OP: **{total_ok}/{total_n} = {sr:.1f}%**")
...
print("\n==================== FINAL RESULTS ====================")
print(f"PoT-FP8 + outlier : {total_ok}/{total_n} = {sr:.1f}%")
