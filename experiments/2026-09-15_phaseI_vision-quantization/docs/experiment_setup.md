...
- **状态**：running（V0 ✅ / V1 ✅ / VLIN ✅ / V2 ✅ / V3 ✅ / V4–V5 待跑 / Vision sparsity-compute ✅：VSC-0 sparsity valid / compute invalid；VSC-1 = 594.41 G / 86.5%）
- **负责人**：
- **创建日期**：2026-09-15
...
统计明确区分量化范围与 raw 范围：Vision 72 Linear 纳入 sparsity；Vision SDPA QK/PV 与 connector 保持 raw，仅纳入 compute accounting，不进入 quantized sparsity denominator。Primary 只跑 Goal task0 × 1ep，并强制 `--skip-calibration` 复用 VLIN 已有 scales。首轮 VSC-0 的 sparsity 数据有效，但复核发现旧 `workload.csv` MatMul MAC shape accounting 错误；exporter 已修复，并已用同一 task 内 VSC-1 变体完成重跑：audited major-op total = 594.41 G、quantized coverage = 86.5%；VSC-0 原始记录不覆盖。
