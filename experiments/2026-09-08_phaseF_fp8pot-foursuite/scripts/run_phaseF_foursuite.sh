...
#
# 用法（迁移至 experiments/ 后，从仓库任意位置执行均可）：
#   bash experiments/2026-09-08_phaseF_fp8pot-foursuite/scripts/run_phaseF_foursuite.sh
# 后台运行：
#   nohup bash experiments/2026-09-08_phaseF_fp8pot-foursuite/scripts/run_phaseF_foursuite.sh > outputs/phaseF_foursuite.log 2>&1 &
set -euo pipefail

# 仓库根 = 本脚本所在 scripts/ 向上三级
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
EXP_DIR="${REPO_ROOT}/experiments/2026-09-08_phaseF_fp8pot-foursuite"
cd "$REPO_ROOT"
...
for exp in "${EXPS[@]}"; do
    for suite in "${SUITES[@]}"; do
        ...
        conda run -n smolvla_eval python main.py \
            --config "${EXP_DIR}/configs/${exp}_${suite}.yaml"
    done
done
