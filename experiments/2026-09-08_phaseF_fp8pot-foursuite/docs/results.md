...
> 注：参照为 ep50 口径（`outputs/verify_libero/*/eval_info.json`），与实验组 ep10 存在口径差；同 suite 内横向对比（F1/F2/F3 之间）不受影响。

## 3. 分组结果与分析

### 3.1 F1 vs F2：outlier 保护的贡献
...
分析：w4（含 1% outlier 元素保护的 int4 权重 + FP8 a/o）掉分高度不均：object 尚可（−9pp），goal 崩溃（21%）。逐 task 看 goal 有 6/10 任务 ≤2/10（见 §3.3），呈任务级选择性失败，提示部分任务对 expert/vlm 权重精度有硬阈值。**w4 不能作为跨 suite 通用配置**；若追求 w4 需按 suite/task 的敏感度做选择性量化或提高保护比例。
