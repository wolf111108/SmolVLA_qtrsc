...
import torch
import yaml
from vla_tcs2.model_wrapper import ModelWrapper, _inject_smolvlm_vision_quantized_matmul, _wrap_smolvlm_vision_linear_layers
...
def main():
    ...
    try:
        ...
        control['quantization']['linear']['enabled'] = False
        ...
        n_linear = _wrap_smolvlm_vision_linear_layers(
            model, cfg['quantization'], 'raw', wrapper.stat_manager)
        linears = [m for m in model.modules() if isinstance(m, QuantizedLinear)]
        expected_linear = {
            f'vision.layers.{i}.{group}.{op}' for i in range(12)
            for group, ops in [('self_attn', ('q_proj','k_proj','v_proj','out_proj')),
                               ('mlp', ('fc1','fc2'))] for op in ops}
        if n_linear != 72 or len(linears) != 72 or {m.module_id for m in linears} != expected_linear:
            raise RuntimeError('Expected exactly 72 Vision Linear modules')
        report['linear_comparisons'] = []
        with torch.no_grad():
            for m in linears:
                x = torch.randn(1, 8, m.in_features, device=m.weight.device, dtype=m.weight.dtype)
                expected = torch.nn.functional.linear(x, m.weight, m.bias)
                actual = m(x)
                record = compare(actual, expected, 1e-6, 1e-5)
                record['module_id'] = m.module_id
                report['linear_comparisons'].append(record)
                save()
        ...
        report['gates']['linear_raw'] = (len(report['linear_comparisons']) == 72 and
            all(r['passed'] for r in report['linear_comparisons']))
        report['gates']['finite'] = complete and all(
            r[name]['nonfinite'] == 0 for r in report['comparisons'] for name in pairs)
        hard_names = ('complete', 'coverage', 'finite', 'adapter_vs_eager', 'linear_raw')
